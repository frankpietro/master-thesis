from src.stats import operator_schedule
...
def new_visit_existing_patient(p_id, skill, duration, input_data, output_data, verbose=False):
    ...
    # op_schedule = operator_schedule(input_data, output_data, assigned_operator, False)

    operators = [i for i in range(input_data['numOperators']) if input_data['operatorSkill'][i] >= skill]
    schedules = {}
    for o in operators:
        schedules[o] = operator_schedule(input_data, output_data, o, False)
...
# def change_operator_availability(operator, day, start_time, end_time, input_data, output_data, commuting_data, verbose=False):
...
#     old_schedule = operator_schedule(input_data, output_data, operator, False)
#     old_visits = [x for x in old_schedule if x[c.DAY] == day and (x[c.START_TIME] < start_time or x[c.END_TIME] > end_time)]

#     # these visits need rescheduling
